...
import { Trash2, Calendar, Clock, Phone, User, Save, RefreshCw, Plus, X, Archive, History } from 'lucide-react';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  // Filter Appointments based on History Toggle
  const now = new Date();
  ...
  const groupedAppointments = [...filteredAppointments]
    .sort((a, b) => {
      ...
      // Sort descending for history, ascending for upcoming
      ...
      {activeTab === 'appointments' ? (
        <div className="space-y-6">
           ...
           <div className="flex justify-end">
              ...
              >
                 {showHistory ? <History size={16} /> : <Archive size={16} />}
          ...
          {Object.keys(groupedAppointments).length === 0 ? (
            <div className="text-center py-12 text-gray-500">
              ...
              <p>{showHistory ? 'אין היסטוריית תורים' : 'אין תורים עתידיים'}</p>
            </div>
          ) : (
            Object.entries(groupedAppointments).map(([date, dayAppts]: [string, Appointment[]]) => {
              ...
              return (
                <div key={date} className={`rounded-2xl overflow-hidden border ${showHistory ? 'bg-dark-800/50 border-white/5 grayscale' : 'bg-dark-800 border-white/10'}`}>
                  <div className="bg-dark-700/50 px-4 py-3 border-b border-white/5 flex justify-between items-center">
                    <h3 className={`font-bold ${showHistory ? 'text-gray-500' : 'text-gold-500'}`}>
