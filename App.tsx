...
function App() {
  ...
  // Automatic Token Sync: Whenever user is logged in + permission granted
  useEffect(() => {
    if (user && notifPermission === 'granted') {
      console.log('🔄 Auto-syncing FCM token for user:', user.phoneNumber);
      messagingService.requestAndSaveToken(user.id, user.phoneNumber);
    }
